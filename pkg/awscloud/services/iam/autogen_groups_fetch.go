...
func FetchGroup(ctx context.Context, params *awscloud.AwsFetchInput) *awscloud.AwsFetchOutput {
	...
	for paginator.HasMorePages() {
		...
		for _, object := range output.Groups {
			...
			if err != nil {
				fetchingErrors = append(fetchingErrors, fmt.Errorf("error storing Group model in %s/%s: %w", params.AccountId, params.Region, err))
			}
			fetchedResources++
		}
