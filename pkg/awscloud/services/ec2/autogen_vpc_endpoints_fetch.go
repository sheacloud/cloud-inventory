...
func FetchVpcEndpoint(ctx context.Context, params *awscloud.AwsFetchInput) *awscloud.AwsFetchOutput {
	...
	for paginator.HasMorePages() {
		...
		for _, object := range output.VpcEndpoints {
			...
			if err != nil {
				fetchingErrors = append(fetchingErrors, fmt.Errorf("error storing VpcEndpoint model in %s/%s: %w", params.AccountId, params.Region, err))
			}
			fetchedResources++
		}
