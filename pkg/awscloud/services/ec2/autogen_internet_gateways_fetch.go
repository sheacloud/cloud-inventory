...
func FetchInternetGateway(ctx context.Context, params *awscloud.AwsFetchInput) *awscloud.AwsFetchOutput {
	...
	for paginator.HasMorePages() {
		...
		if err != nil {
			fetchingErrors = append(fetchingErrors, fmt.Errorf("error calling DescribeInternetGateways in %s/%s: %w", params.AccountId, params.Region, err))
			break
		}
		...
		for _, object := range output.InternetGateways {
			...
			if err != nil {
				fetchingErrors = append(fetchingErrors, fmt.Errorf("error storing InternetGateway model in %s/%s: %w", params.AccountId, params.Region, err))
			}
			fetchedResources++
		}
