...
func FetchBackupPlans(ctx context.Context, params *aws.AwsFetchInput) ([]*BackupPlan, *aws.AwsFetchOutputMetadata) {
	...
	for paginator.HasMorePages() {
		...
		if err != nil {
			fetchingErrors = append(fetchingErrors, fmt.Errorf("error calling ListBackupPlans in %s/%s: %w", params.AccountId, params.Region, err))
			break
		}
		...
		for _, object := range output.BackupPlansList {
			...
			if err = PostProcessBackupPlan(ctx, params, model); err != nil {
				fetchingErrors = append(fetchingErrors, fmt.Errorf("error post-processing BackupPlan %s %s/%s: %w", model.BackupPlanArn, params.AccountId, params.Region, err))
				failedResources++
			}
