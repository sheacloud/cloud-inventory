...
func GetBucketsMetadata(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	reportDate := c.Query("report_date")
	if reportDate == "" {
		reportDate = time.Now().UTC().Format("2006-01-02")
	}

	s3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "s3", "buckets"}, reportDate, indexedstorage.RequestTimeSelection{}, s3Client, new(s3.Bucket))
	if err != nil {
...
// @Security     ApiKeyAuth
// @Success      200  {array}   s3.Bucket
// @Failure      400
// @Router       /inventory/aws/s3/buckets [get]
func ListBuckets(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	...
	s3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "s3", "buckets"}, *params.ReportDate, params.GetRequestTimeSelection(), s3Client, new(s3.Bucket))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
...
func GetBucket(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	var params routes.AwsQueryParameters
	if err := c.BindQuery(&params); err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
...
func DiffMultiBuckets(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	...
	startS3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "s3", "buckets"}, *params.StartReportDate, params.GetRequestStartTimeSelection(), s3Client, new(s3.Bucket))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
	...
	endS3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "s3", "buckets"}, *params.EndReportDate, params.GetRequestEndTimeSelection(), s3Client, new(s3.Bucket))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
...
func DiffSingleBucket(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	var params routes.AwsDiffParameters
	if err := c.BindQuery(&params); err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
