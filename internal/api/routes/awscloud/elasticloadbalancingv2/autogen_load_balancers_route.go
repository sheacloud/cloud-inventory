...
func GetLoadBalancersMetadata(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	...
	s3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "elasticloadbalancingv2", "load_balancers"}, reportDate, indexedstorage.RequestTimeSelection{}, s3Client, new(elasticloadbalancingv2.LoadBalancer))
	if err != nil {
...
func DiffSingleLoadBalancer(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	var params routes.AwsDiffParameters
	if err := c.BindQuery(&params); err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	...
	var startObject *elasticloadbalancingv2.LoadBalancer
	startS3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "elasticloadbalancingv2", "load_balancers"}, *params.StartReportDate, params.GetRequestStartTimeSelection(), s3Client, new(elasticloadbalancingv2.LoadBalancer))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	...
	var endObject *elasticloadbalancingv2.LoadBalancer
	endS3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "elasticloadbalancingv2", "load_balancers"}, *params.EndReportDate, params.GetRequestEndTimeSelection(), s3Client, new(elasticloadbalancingv2.LoadBalancer))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
