...
func GetFunctionsMetadata(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	reportDate := c.Query("report_date")
	if reportDate == "" {
		reportDate = time.Now().UTC().Format("2006-01-02")
	}

	s3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "lambda", "functions"}, reportDate, indexedstorage.RequestTimeSelection{}, s3Client, new(lambda.FunctionConfiguration))
	if err != nil {
...
// @Success      200  {array}   lambda.FunctionConfiguration
// @Failure      400
// @Router       /inventory/aws/lambda/functions [get]
func ListFunctions(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	...
	s3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "lambda", "functions"}, *params.ReportDate, params.GetRequestTimeSelection(), s3Client, new(lambda.FunctionConfiguration))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
	...
	if params.AccountId != nil || params.Region != nil {
		...
		for _, result := range results {
			obj := result.(*lambda.FunctionConfiguration)
...
// GetFunctionConfiguration godoc
// @Summary      Get a specific FunctionConfiguration
// @Description  Get a specific FunctionConfiguration by its FunctionArn
// @Tags         aws lambda
// @Produce      json
// @Param        report_date query string false  "Which date to pull data from. Current date by default" Format(date)
// @Param        function_arn path string true "The function_arn of the FunctionConfiguration to retrieve"
// @Param		 account_id query string false  "A specific account to pull data from. All accounts by default"
// @Param		 region query string false  "A specific region to pull data from. All regions by default"
// @Param		 time_selection query string false  "How to select the time range to pull data from. 'latest' by default" Enums(latest, before, after, at)
// @Param		 time_selection_reference query string false  "The reference time to use when selecting the time range to pull data from. Only used when time_selection is 'before', 'after', or 'at'." Format(dateTime)
// @Success      200  {object}   lambda.FunctionConfiguration
// @Failure      400
// @Failure 	 404
// @Router       /inventory/aws/lambda/functions/{function_arn} [get]
func GetFunctionConfiguration(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	var params routes.AwsQueryParameters
	if err := c.BindQuery(&params); err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	...
	for _, result := range results {
		...
		if obj.FunctionArn == id {
			c.IndentedJSON(200, obj)
			return
		}
	}
	c.AbortWithStatusJSON(404, gin.H{"error": "No FunctionConfiguration found with function_arn " + id})
}
...
func DiffMultiFunctions(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	...
	startS3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "lambda", "functions"}, *params.StartReportDate, params.GetRequestStartTimeSelection(), s3Client, new(lambda.FunctionConfiguration))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
	...
	endS3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "lambda", "functions"}, *params.EndReportDate, params.GetRequestEndTimeSelection(), s3Client, new(lambda.FunctionConfiguration))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
...
// DiffSingleFunctionConfiguration godoc
...
func DiffSingleFunctionConfiguration(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	var params routes.AwsDiffParameters
	if err := c.BindQuery(&params); err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	...
	var startObject *lambda.FunctionConfiguration
	startS3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "lambda", "functions"}, *params.StartReportDate, params.GetRequestStartTimeSelection(), s3Client, new(lambda.FunctionConfiguration))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	...
	var endObject *lambda.FunctionConfiguration
	endS3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "lambda", "functions"}, *params.EndReportDate, params.GetRequestEndTimeSelection(), s3Client, new(lambda.FunctionConfiguration))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
