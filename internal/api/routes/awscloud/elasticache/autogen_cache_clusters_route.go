...
func GetCacheClustersMetadata(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	reportDate := c.Query("report_date")
	if reportDate == "" {
		reportDate = time.Now().UTC().Format("2006-01-02")
	}

	s3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "elasticache", "cache_clusters"}, reportDate, indexedstorage.RequestTimeSelection{}, s3Client, new(elasticache.CacheCluster))
	if err != nil {
