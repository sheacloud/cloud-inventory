...
func GetDBInstancesMetadata(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	reportDate := c.Query("report_date")
	if reportDate == "" {
		reportDate = time.Now().UTC().Format("2006-01-02")
	}

	s3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "rds", "db_instances"}, reportDate, indexedstorage.RequestTimeSelection{}, s3Client, new(rds.DBInstance))
	if err != nil {
...
func DiffMultiDBInstances(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
...
func DiffSingleDBInstance(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	var params routes.AwsDiffParameters
	if err := c.BindQuery(&params); err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	...
	var startObject *rds.DBInstance
	startS3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "rds", "db_instances"}, *params.StartReportDate, params.GetRequestStartTimeSelection(), s3Client, new(rds.DBInstance))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
