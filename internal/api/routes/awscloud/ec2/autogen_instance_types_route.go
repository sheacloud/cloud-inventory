...
// GetInstanceTypesMetadata godoc
...
// @Failure      400
// @Router       /metadata/aws/ec2/instance_types [get]
func GetInstanceTypesMetadata(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	reportDate := c.Query("report_date")
	if reportDate == "" {
		reportDate = time.Now().UTC().Format("2006-01-02")
	}

	s3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "ec2", "instance_types"}, reportDate, indexedstorage.RequestTimeSelection{}, s3Client, new(ec2.InstanceTypeInfo))
	if err != nil {
...
func DiffMultiInstanceTypes(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
...
// DiffSingleInstanceTypeInfo godoc
...
// @Success      200  {array}   routes.Diff
// @Failure      400
// @Router       /diff/aws/ec2/instance_types/{instance_type} [get]
func DiffSingleInstanceTypeInfo(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	var params routes.AwsDiffParameters
	if err := c.BindQuery(&params); err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	...
	var startObject *ec2.InstanceTypeInfo
	startS3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "ec2", "instance_types"}, *params.StartReportDate, params.GetRequestStartTimeSelection(), s3Client, new(ec2.InstanceTypeInfo))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	...
	var endObject *ec2.InstanceTypeInfo
	endS3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "ec2", "instance_types"}, *params.EndReportDate, params.GetRequestEndTimeSelection(), s3Client, new(ec2.InstanceTypeInfo))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
