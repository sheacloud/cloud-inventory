...
func GetTransitGatewayRouteTablesMetadata(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	reportDate := c.Query("report_date")
	if reportDate == "" {
		reportDate = time.Now().UTC().Format("2006-01-02")
	}

	s3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "ec2", "transit_gateway_route_tables"}, reportDate, indexedstorage.RequestTimeSelection{}, s3Client, new(ec2.TransitGatewayRouteTable))
	if err != nil {
...
// @Security     ApiKeyAuth
// @Success      200  {array}   ec2.TransitGatewayRouteTable
// @Failure      400
// @Router       /inventory/aws/ec2/transit_gateway_route_tables [get]
func ListTransitGatewayRouteTables(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	...
	s3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "ec2", "transit_gateway_route_tables"}, *params.ReportDate, params.GetRequestTimeSelection(), s3Client, new(ec2.TransitGatewayRouteTable))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
	...
	if params.AccountId != nil || params.Region != nil {
		...
		for _, result := range results {
			obj := result.(*ec2.TransitGatewayRouteTable)
...
func GetTransitGatewayRouteTable(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	var params routes.AwsQueryParameters
	if err := c.BindQuery(&params); err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	...
	id := c.Param("transit_gateway_route_table_id")
	...
	for _, result := range results {
		...
		if obj.TransitGatewayRouteTableId == id {
...
func DiffMultiTransitGatewayRouteTables(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	...
	startS3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "ec2", "transit_gateway_route_tables"}, *params.StartReportDate, params.GetRequestStartTimeSelection(), s3Client, new(ec2.TransitGatewayRouteTable))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
	...
	endS3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "ec2", "transit_gateway_route_tables"}, *params.EndReportDate, params.GetRequestEndTimeSelection(), s3Client, new(ec2.TransitGatewayRouteTable))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
...
func DiffSingleTransitGatewayRouteTable(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	var params routes.AwsDiffParameters
	if err := c.BindQuery(&params); err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	...
	var startObject *ec2.TransitGatewayRouteTable
	startS3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "ec2", "transit_gateway_route_tables"}, *params.StartReportDate, params.GetRequestStartTimeSelection(), s3Client, new(ec2.TransitGatewayRouteTable))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	...
	var endObject *ec2.TransitGatewayRouteTable
	endS3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "ec2", "transit_gateway_route_tables"}, *params.EndReportDate, params.GetRequestEndTimeSelection(), s3Client, new(ec2.TransitGatewayRouteTable))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
