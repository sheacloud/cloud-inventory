...
func GetSecurityGroupsMetadata(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	reportDate := c.Query("report_date")
	if reportDate == "" {
		reportDate = time.Now().UTC().Format("2006-01-02")
	}

	s3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "ec2", "security_groups"}, reportDate, indexedstorage.RequestTimeSelection{}, s3Client, new(ec2.SecurityGroup))
	if err != nil {
