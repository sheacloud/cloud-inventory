...
func GetSubnetsMetadata(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	reportDate := c.Query("report_date")
	if reportDate == "" {
		reportDate = time.Now().UTC().Format("2006-01-02")
	}

	s3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "ec2", "subnets"}, reportDate, indexedstorage.RequestTimeSelection{}, s3Client, new(ec2.Subnet))
	if err != nil {
...
// @Success      200  {array}   ec2.Subnet
// @Failure      400
// @Router       /inventory/aws/ec2/subnets [get]
func ListSubnets(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	...
	s3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "ec2", "subnets"}, *params.ReportDate, params.GetRequestTimeSelection(), s3Client, new(ec2.Subnet))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
...
func GetSubnet(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	var params routes.AwsQueryParameters
	if err := c.BindQuery(&params); err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
...
func DiffMultiSubnets(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	...
	startS3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "ec2", "subnets"}, *params.StartReportDate, params.GetRequestStartTimeSelection(), s3Client, new(ec2.Subnet))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
	...
	endS3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "ec2", "subnets"}, *params.EndReportDate, params.GetRequestEndTimeSelection(), s3Client, new(ec2.Subnet))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
...
func DiffSingleSubnet(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	var params routes.AwsDiffParameters
	if err := c.BindQuery(&params); err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
