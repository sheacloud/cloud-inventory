...
func GetVolumesMetadata(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	reportDate := c.Query("report_date")
	if reportDate == "" {
		reportDate = time.Now().UTC().Format("2006-01-02")
	}

	s3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "ec2", "volumes"}, reportDate, indexedstorage.RequestTimeSelection{}, s3Client, new(ec2.Volume))
	if err != nil {
...
// @Security     ApiKeyAuth
// @Success      200  {array}   ec2.Volume
// @Failure      400
// @Router       /inventory/aws/ec2/volumes [get]
func ListVolumes(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	...
	s3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "ec2", "volumes"}, *params.ReportDate, params.GetRequestTimeSelection(), s3Client, new(ec2.Volume))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
...
func GetVolume(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	var params routes.AwsQueryParameters
	if err := c.BindQuery(&params); err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
...
func DiffMultiVolumes(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	...
	startS3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "ec2", "volumes"}, *params.StartReportDate, params.GetRequestStartTimeSelection(), s3Client, new(ec2.Volume))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
	...
	endS3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "ec2", "volumes"}, *params.EndReportDate, params.GetRequestEndTimeSelection(), s3Client, new(ec2.Volume))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
...
func DiffSingleVolume(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	var params routes.AwsDiffParameters
	if err := c.BindQuery(&params); err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
