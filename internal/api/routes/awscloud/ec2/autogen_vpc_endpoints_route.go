...
func GetVpcEndpointsMetadata(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	reportDate := c.Query("report_date")
	if reportDate == "" {
		reportDate = time.Now().UTC().Format("2006-01-02")
	}

	s3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "ec2", "vpc_endpoints"}, reportDate, indexedstorage.RequestTimeSelection{}, s3Client, new(ec2.VpcEndpoint))
	if err != nil {
...
func ListVpcEndpoints(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	...
	s3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "ec2", "vpc_endpoints"}, *params.ReportDate, params.GetRequestTimeSelection(), s3Client, new(ec2.VpcEndpoint))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
...
func GetVpcEndpoint(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	var params routes.AwsQueryParameters
	if err := c.BindQuery(&params); err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
...
func DiffMultiVpcEndpoints(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	...
	startS3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "ec2", "vpc_endpoints"}, *params.StartReportDate, params.GetRequestStartTimeSelection(), s3Client, new(ec2.VpcEndpoint))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
	...
	endS3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "ec2", "vpc_endpoints"}, *params.EndReportDate, params.GetRequestEndTimeSelection(), s3Client, new(ec2.VpcEndpoint))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
...
func DiffSingleVpcEndpoint(c *gin.Context, s3Client *awsS3.Client, s3Bucket string) {
	var params routes.AwsDiffParameters
	if err := c.BindQuery(&params); err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	...
	var startObject *ec2.VpcEndpoint
	startS3DirReader, err := indexedstorage.NewParquetS3DirectoryReader(c.Request.Context(), s3Bucket, []string{"aws", "ec2", "vpc_endpoints"}, *params.StartReportDate, params.GetRequestStartTimeSelection(), s3Client, new(ec2.VpcEndpoint))
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
