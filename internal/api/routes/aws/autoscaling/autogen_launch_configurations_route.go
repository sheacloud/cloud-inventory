...
// GetLaunchConfigurationsMetadata godoc
// @Summary      Get LaunchConfigurations Metadata
// @Description  get a list of launch_configurations metadata
...
// @Failure      400
// @Router       /metadata/aws/autoscaling/launch_configurations [get]
func GetLaunchConfigurationsMetadata(c *gin.Context, dao db.ReaderDAO) {
	...
	reportTimes, err := dao.GetAwsAutoScalingLaunchConfigurationReportTimes(c, reportDate.UnixMilli())
	if err != nil {
	...
	c.IndentedJSON(200, routes.AwsResourceMetadata{
		...
		IdField:   "launch_configuration_arn",
...
// ListLaunchConfigurations godoc
// @Summary      List LaunchConfigurations
// @Description  get a list of launch_configurations
// @Tags         aws autoscaling
// @Produce      json
// @Param        report_date query string false  "Which date to pull data from. Current date by default" Format(date)
...
// @Security     ApiKeyAuth
// @Success      200  {object}   ListLaunchConfigurationsResponse
// @Failure      400
// @Router       /inventory/aws/autoscaling/launch_configurations [get]
func ListLaunchConfigurations(c *gin.Context, dao db.ReaderDAO) {
	...
	selectedTime, err := dao.GetReferencedAwsAutoScalingLaunchConfigurationReportTime(c, params.ReportDateUnixMilli, *params.TimeSelection, params.TimeSelectionReference)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}

	results, err := dao.ListAwsAutoScalingLaunchConfigurations(c, *selectedTime, params.AccountId, params.Region, nil, nil)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}

	c.IndentedJSON(200, ListLaunchConfigurationsResponse{
		LaunchConfigurations: results,
	})
}

// GetLaunchConfiguration godoc
// @Summary      Get a specific LaunchConfiguration
// @Description  Get a specific LaunchConfiguration by its LaunchConfigurationARN
// @Tags         aws autoscaling
// @Produce      json
// @Param        report_date query string false  "Which date to pull data from. Current date by default" Format(date)
// @Param        launch_configuration_arn path string true "The launch_configuration_arn of the LaunchConfiguration to retrieve"
// @Param		 account_id query string false  "A specific account to pull data from. All accounts by default"
// @Param		 region query string false  "A specific region to pull data from. All regions by default"
// @Param		 time_selection query string false  "How to select the time range to pull data from. 'latest' by default" Enums(latest, before, after, at)
// @Param		 time_selection_reference query string false  "The reference time to use when selecting the time range to pull data from. Only used when time_selection is 'before', 'after', or 'at'." Format(dateTime)
// @Security     ApiKeyAuth
// @Success      200  {object}   autoscaling.LaunchConfiguration
// @Failure      400
// @Failure 	 404
// @Router       /inventory/aws/autoscaling/launch_configurations/{launch_configuration_arn} [get]
func GetLaunchConfiguration(c *gin.Context, dao db.ReaderDAO) {
	...
	id, err := url.QueryUnescape(c.Param("launch_configuration_arn"))
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}
	...
	result, err := dao.GetAwsAutoScalingLaunchConfiguration(c, *selectedTime, id)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}
...
// @Description  get a diff of LaunchConfigurations between two points in time
// @Tags         aws autoscaling
// @Produce      json
...
// @Failure      400
// @Router       /diff/aws/autoscaling/launch_configurations [get]
func DiffMultiLaunchConfigurations(c *gin.Context, dao db.ReaderDAO) {
	...
	startSelectedTime, err := dao.GetReferencedAwsAutoScalingLaunchConfigurationReportTime(c, params.StartReportDateUnixMilli, *params.StartTimeSelection, params.StartTimeSelectionReference)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}

	startResults, err := dao.ListAwsAutoScalingLaunchConfigurations(c, *startSelectedTime, params.AccountId, params.Region, nil, nil)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}

	endSelectedTime, err := dao.GetReferencedAwsAutoScalingLaunchConfigurationReportTime(c, params.EndReportDateUnixMilli, *params.EndTimeSelection, params.EndTimeSelectionReference)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}

	endResults, err := dao.ListAwsAutoScalingLaunchConfigurations(c, *endSelectedTime, params.AccountId, params.Region, nil, nil)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
...
// DiffSingleLaunchConfiguration godoc
// @Summary      Diff LaunchConfiguration
// @Description  get a diff of LaunchConfiguration between two points in time
// @Tags         aws autoscaling
// @Produce      json
// @Param        start_report_date query string true  "Which date to pull data from. Current date by default" Format(date)
// @Param		 start_time_selection query string false  "How to select the time range to pull data from. 'latest' by default" Enums(latest, before, after, at)
...
// @Success      200  {array}   routes.Diff
// @Failure      400
// @Router       /diff/aws/autoscaling/launch_configurations/{launch_configuration_arn} [get]
func DiffSingleLaunchConfiguration(c *gin.Context, dao db.ReaderDAO) {
	...
	startObject, err := dao.GetAwsAutoScalingLaunchConfiguration(c, *startSelectedTime, id)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
	...
	endObject, err := dao.GetAwsAutoScalingLaunchConfiguration(c, *endSelectedTime, id)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
	...
	if startObject == nil && endObject == nil {
		c.AbortWithStatusJSON(404, gin.H{"error": "No LaunchConfiguration found with launch_configuration_arn " + id})
		return
	} else {
