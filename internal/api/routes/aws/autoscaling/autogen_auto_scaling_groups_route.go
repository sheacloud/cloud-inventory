...
// GetAutoScalingGroupsMetadata godoc
// @Summary      Get AutoScalingGroups Metadata
// @Description  get a list of auto_scaling_groups metadata
...
// @Failure      400
// @Router       /metadata/aws/autoscaling/auto_scaling_groups [get]
func GetAutoScalingGroupsMetadata(c *gin.Context, dao db.ReaderDAO) {
	...
	reportTimes, err := dao.GetAwsAutoScalingAutoScalingGroupReportTimes(c, reportDate.UnixMilli())
	if err != nil {
	...
	c.IndentedJSON(200, routes.AwsResourceMetadata{
		...
		IdField:   "auto_scaling_group_arn",
...
// @Summary      List AutoScalingGroups
// @Description  get a list of auto_scaling_groups
// @Tags         aws autoscaling
// @Produce      json
// @Param        report_date query string false  "Which date to pull data from. Current date by default" Format(date)
...
// @Security     ApiKeyAuth
// @Success      200  {object}   ListAutoScalingGroupsResponse
// @Failure      400
// @Router       /inventory/aws/autoscaling/auto_scaling_groups [get]
func ListAutoScalingGroups(c *gin.Context, dao db.ReaderDAO) {
	...
	selectedTime, err := dao.GetReferencedAwsAutoScalingAutoScalingGroupReportTime(c, params.ReportDateUnixMilli, *params.TimeSelection, params.TimeSelectionReference)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}

	results, err := dao.ListAwsAutoScalingAutoScalingGroups(c, *selectedTime, params.AccountId, params.Region, nil, nil)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}

	c.IndentedJSON(200, ListAutoScalingGroupsResponse{
		AutoScalingGroups: results,
	})
}

// GetAutoScalingGroup godoc
// @Summary      Get a specific AutoScalingGroup
// @Description  Get a specific AutoScalingGroup by its AutoScalingGroupARN
// @Tags         aws autoscaling
// @Produce      json
// @Param        report_date query string false  "Which date to pull data from. Current date by default" Format(date)
// @Param        auto_scaling_group_arn path string true "The auto_scaling_group_arn of the AutoScalingGroup to retrieve"
// @Param		 account_id query string false  "A specific account to pull data from. All accounts by default"
// @Param		 region query string false  "A specific region to pull data from. All regions by default"
// @Param		 time_selection query string false  "How to select the time range to pull data from. 'latest' by default" Enums(latest, before, after, at)
// @Param		 time_selection_reference query string false  "The reference time to use when selecting the time range to pull data from. Only used when time_selection is 'before', 'after', or 'at'." Format(dateTime)
// @Security     ApiKeyAuth
// @Success      200  {object}   autoscaling.AutoScalingGroup
// @Failure      400
// @Failure 	 404
// @Router       /inventory/aws/autoscaling/auto_scaling_groups/{auto_scaling_group_arn} [get]
func GetAutoScalingGroup(c *gin.Context, dao db.ReaderDAO) {
	...
	id, err := url.QueryUnescape(c.Param("auto_scaling_group_arn"))
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}
	...
	result, err := dao.GetAwsAutoScalingAutoScalingGroup(c, *selectedTime, id)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}
...
// DiffMultiAutoScalingGroups godoc
// @Summary      Diff AutoScalingGroups
// @Description  get a diff of AutoScalingGroups between two points in time
// @Tags         aws autoscaling
// @Produce      json
...
// @Failure      400
// @Router       /diff/aws/autoscaling/auto_scaling_groups [get]
func DiffMultiAutoScalingGroups(c *gin.Context, dao db.ReaderDAO) {
	...
	startSelectedTime, err := dao.GetReferencedAwsAutoScalingAutoScalingGroupReportTime(c, params.StartReportDateUnixMilli, *params.StartTimeSelection, params.StartTimeSelectionReference)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}

	startResults, err := dao.ListAwsAutoScalingAutoScalingGroups(c, *startSelectedTime, params.AccountId, params.Region, nil, nil)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}

	endSelectedTime, err := dao.GetReferencedAwsAutoScalingAutoScalingGroupReportTime(c, params.EndReportDateUnixMilli, *params.EndTimeSelection, params.EndTimeSelectionReference)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}

	endResults, err := dao.ListAwsAutoScalingAutoScalingGroups(c, *endSelectedTime, params.AccountId, params.Region, nil, nil)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
...
// DiffSingleAutoScalingGroup godoc
// @Summary      Diff AutoScalingGroup
// @Description  get a diff of AutoScalingGroup between two points in time
// @Tags         aws autoscaling
// @Produce      json
// @Param        start_report_date query string true  "Which date to pull data from. Current date by default" Format(date)
// @Param		 start_time_selection query string false  "How to select the time range to pull data from. 'latest' by default" Enums(latest, before, after, at)
...
// @Success      200  {array}   routes.Diff
// @Failure      400
// @Router       /diff/aws/autoscaling/auto_scaling_groups/{auto_scaling_group_arn} [get]
func DiffSingleAutoScalingGroup(c *gin.Context, dao db.ReaderDAO) {
	...
	startObject, err := dao.GetAwsAutoScalingAutoScalingGroup(c, *startSelectedTime, id)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
	...
	endObject, err := dao.GetAwsAutoScalingAutoScalingGroup(c, *endSelectedTime, id)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
	...
	if startObject == nil && endObject == nil {
		c.AbortWithStatusJSON(404, gin.H{"error": "No AutoScalingGroup found with auto_scaling_group_arn " + id})
		return
	} else {
