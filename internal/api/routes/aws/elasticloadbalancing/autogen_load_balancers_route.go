...
type ListLoadBalancersResponse struct {
	LoadBalancers   []*elasticloadbalancing.LoadBalancer `json:"load_balancers"`
...
func GetLoadBalancersMetadata(c *gin.Context, dao db.ReaderDAO) {
	...
	reportTimes, err := dao.GetAwsElasticLoadBalancingLoadBalancerReportTimes(c, reportDate.UnixMilli())
	if err != nil {
...
func ListLoadBalancers(c *gin.Context, dao db.ReaderDAO) {
	...
	selectedTime, err := dao.GetReferencedAwsElasticLoadBalancingLoadBalancerReportTime(c, params.ReportDateUnixMilli, *params.TimeSelection, params.TimeSelectionReference)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}

	results, err := dao.ListAwsElasticLoadBalancingLoadBalancers(c, *selectedTime, params.AccountId, params.Region, nil, nil)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}

	c.IndentedJSON(200, ListLoadBalancersResponse{
...
// @Produce      json
// @Param        report_date query string false  "Which date to pull data from. Current date by default" Format(date)
// @Param        load_balancer_name path string true "The load_balancer_name of the LoadBalancer to retrieve"
// @Param		 account_id query string false  "A specific account to pull data from. All accounts by default"
// @Param		 region query string false  "A specific region to pull data from. All regions by default"
// @Param		 time_selection query string false  "How to select the time range to pull data from. 'latest' by default" Enums(latest, before, after, at)
// @Param		 time_selection_reference query string false  "The reference time to use when selecting the time range to pull data from. Only used when time_selection is 'before', 'after', or 'at'." Format(dateTime)
// @Security     ApiKeyAuth
// @Success      200  {object}   elasticloadbalancing.LoadBalancer
// @Failure      400
// @Failure 	 404
...
func GetLoadBalancer(c *gin.Context, dao db.ReaderDAO) {
	...
	id, err := url.QueryUnescape(c.Param("load_balancer_name"))
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}
	...
	result, err := dao.GetAwsElasticLoadBalancingLoadBalancer(c, *selectedTime, id)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}
...
func DiffMultiLoadBalancers(c *gin.Context, dao db.ReaderDAO) {
	...
	startSelectedTime, err := dao.GetReferencedAwsElasticLoadBalancingLoadBalancerReportTime(c, params.StartReportDateUnixMilli, *params.StartTimeSelection, params.StartTimeSelectionReference)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}

	startResults, err := dao.ListAwsElasticLoadBalancingLoadBalancers(c, *startSelectedTime, params.AccountId, params.Region, nil, nil)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}

	endSelectedTime, err := dao.GetReferencedAwsElasticLoadBalancingLoadBalancerReportTime(c, params.EndReportDateUnixMilli, *params.EndTimeSelection, params.EndTimeSelectionReference)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}

	endResults, err := dao.ListAwsElasticLoadBalancingLoadBalancers(c, *endSelectedTime, params.AccountId, params.Region, nil, nil)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
...
func DiffSingleLoadBalancer(c *gin.Context, dao db.ReaderDAO) {
	...
	startObject, err := dao.GetAwsElasticLoadBalancingLoadBalancer(c, *startSelectedTime, id)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
	...
	endObject, err := dao.GetAwsElasticLoadBalancingLoadBalancer(c, *endSelectedTime, id)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
	...
	if startObject == nil && endObject == nil {
		c.AbortWithStatusJSON(404, gin.H{"error": "No LoadBalancer found with load_balancer_name " + id})
		return
	} else {
