...
// GetDataCatalogsMetadata godoc
// @Summary      Get DataCatalogs Metadata
// @Description  get a list of data_catalogs metadata
// @Tags         aws athena
// @Produce      json
...
// @Failure      400
// @Router       /metadata/aws/athena/data_catalogs [get]
func GetDataCatalogsMetadata(c *gin.Context, dao db.ReaderDAO) {
	...
	reportTimes, err := dao.GetAwsAthenaDataCatalogReportTimes(c, reportDate.UnixMilli())
	if err != nil {
...
// ListDataCatalogs godoc
// @Summary      List DataCatalogs
// @Description  get a list of data_catalogs
// @Tags         aws athena
// @Produce      json
// @Param        report_date query string false  "Which date to pull data from. Current date by default" Format(date)
...
// @Security     ApiKeyAuth
// @Success      200  {object}   ListDataCatalogsResponse
// @Failure      400
// @Router       /inventory/aws/athena/data_catalogs [get]
func ListDataCatalogs(c *gin.Context, dao db.ReaderDAO) {
	...
	selectedTime, err := dao.GetReferencedAwsAthenaDataCatalogReportTime(c, params.ReportDateUnixMilli, *params.TimeSelection, params.TimeSelectionReference)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}

	results, err := dao.ListAwsAthenaDataCatalogs(c, *selectedTime, params.AccountId, params.Region, nil, nil)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}

	c.IndentedJSON(200, ListDataCatalogsResponse{
		DataCatalogs: results,
	})
}

// GetDataCatalog godoc
// @Summary      Get a specific DataCatalog
// @Description  Get a specific DataCatalog by its Name
// @Tags         aws athena
// @Produce      json
// @Param        report_date query string false  "Which date to pull data from. Current date by default" Format(date)
// @Param        name path string true "The name of the DataCatalog to retrieve"
// @Param		 account_id query string false  "A specific account to pull data from. All accounts by default"
// @Param		 region query string false  "A specific region to pull data from. All regions by default"
// @Param		 time_selection query string false  "How to select the time range to pull data from. 'latest' by default" Enums(latest, before, after, at)
// @Param		 time_selection_reference query string false  "The reference time to use when selecting the time range to pull data from. Only used when time_selection is 'before', 'after', or 'at'." Format(dateTime)
// @Security     ApiKeyAuth
// @Success      200  {object}   athena.DataCatalog
// @Failure      400
// @Failure 	 404
// @Router       /inventory/aws/athena/data_catalogs/{name} [get]
func GetDataCatalog(c *gin.Context, dao db.ReaderDAO) {
	...
	result, err := dao.GetAwsAthenaDataCatalog(c, *selectedTime, id)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}
...
// DiffMultiDataCatalogs godoc
// @Summary      Diff DataCatalogs
// @Description  get a diff of DataCatalogs between two points in time
// @Tags         aws athena
// @Produce      json
...
// @Failure      400
// @Router       /diff/aws/athena/data_catalogs [get]
func DiffMultiDataCatalogs(c *gin.Context, dao db.ReaderDAO) {
	...
	startSelectedTime, err := dao.GetReferencedAwsAthenaDataCatalogReportTime(c, params.StartReportDateUnixMilli, *params.StartTimeSelection, params.StartTimeSelectionReference)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}

	startResults, err := dao.ListAwsAthenaDataCatalogs(c, *startSelectedTime, params.AccountId, params.Region, nil, nil)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}

	endSelectedTime, err := dao.GetReferencedAwsAthenaDataCatalogReportTime(c, params.EndReportDateUnixMilli, *params.EndTimeSelection, params.EndTimeSelectionReference)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}

	endResults, err := dao.ListAwsAthenaDataCatalogs(c, *endSelectedTime, params.AccountId, params.Region, nil, nil)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
...
// DiffSingleDataCatalog godoc
// @Summary      Diff DataCatalog
// @Description  get a diff of DataCatalog between two points in time
// @Tags         aws athena
// @Produce      json
// @Param        start_report_date query string true  "Which date to pull data from. Current date by default" Format(date)
// @Param		 start_time_selection query string false  "How to select the time range to pull data from. 'latest' by default" Enums(latest, before, after, at)
...
// @Success      200  {array}   routes.Diff
// @Failure      400
// @Router       /diff/aws/athena/data_catalogs/{name} [get]
func DiffSingleDataCatalog(c *gin.Context, dao db.ReaderDAO) {
	...
	startObject, err := dao.GetAwsAthenaDataCatalog(c, *startSelectedTime, id)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
	...
	endObject, err := dao.GetAwsAthenaDataCatalog(c, *endSelectedTime, id)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
	...
	if startObject == nil && endObject == nil {
		c.AbortWithStatusJSON(404, gin.H{"error": "No DataCatalog found with name " + id})
		return
	} else {
