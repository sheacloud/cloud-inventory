...
// GetWorkGroupsMetadata godoc
// @Summary      Get WorkGroups Metadata
// @Description  get a list of work_groups metadata
// @Tags         aws athena
// @Produce      json
...
// @Failure      400
// @Router       /metadata/aws/athena/work_groups [get]
func GetWorkGroupsMetadata(c *gin.Context, dao db.ReaderDAO) {
	...
	reportTimes, err := dao.GetAwsAthenaWorkGroupReportTimes(c, reportDate.UnixMilli())
	if err != nil {
...
// ListWorkGroups godoc
// @Summary      List WorkGroups
// @Description  get a list of work_groups
// @Tags         aws athena
// @Produce      json
// @Param        report_date query string false  "Which date to pull data from. Current date by default" Format(date)
...
// @Security     ApiKeyAuth
// @Success      200  {object}   ListWorkGroupsResponse
// @Failure      400
// @Router       /inventory/aws/athena/work_groups [get]
func ListWorkGroups(c *gin.Context, dao db.ReaderDAO) {
	...
	selectedTime, err := dao.GetReferencedAwsAthenaWorkGroupReportTime(c, params.ReportDateUnixMilli, *params.TimeSelection, params.TimeSelectionReference)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}

	results, err := dao.ListAwsAthenaWorkGroups(c, *selectedTime, params.AccountId, params.Region, nil, nil)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}

	c.IndentedJSON(200, ListWorkGroupsResponse{
		WorkGroups: results,
	})
}

// GetWorkGroup godoc
// @Summary      Get a specific WorkGroup
// @Description  Get a specific WorkGroup by its Name
// @Tags         aws athena
// @Produce      json
// @Param        report_date query string false  "Which date to pull data from. Current date by default" Format(date)
// @Param        name path string true "The name of the WorkGroup to retrieve"
// @Param		 account_id query string false  "A specific account to pull data from. All accounts by default"
// @Param		 region query string false  "A specific region to pull data from. All regions by default"
// @Param		 time_selection query string false  "How to select the time range to pull data from. 'latest' by default" Enums(latest, before, after, at)
// @Param		 time_selection_reference query string false  "The reference time to use when selecting the time range to pull data from. Only used when time_selection is 'before', 'after', or 'at'." Format(dateTime)
// @Security     ApiKeyAuth
// @Success      200  {object}   athena.WorkGroup
// @Failure      400
// @Failure 	 404
// @Router       /inventory/aws/athena/work_groups/{name} [get]
func GetWorkGroup(c *gin.Context, dao db.ReaderDAO) {
	...
	result, err := dao.GetAwsAthenaWorkGroup(c, *selectedTime, id)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}
...
// DiffMultiWorkGroups godoc
// @Summary      Diff WorkGroups
// @Description  get a diff of WorkGroups between two points in time
// @Tags         aws athena
// @Produce      json
...
// @Failure      400
// @Router       /diff/aws/athena/work_groups [get]
func DiffMultiWorkGroups(c *gin.Context, dao db.ReaderDAO) {
	...
	startSelectedTime, err := dao.GetReferencedAwsAthenaWorkGroupReportTime(c, params.StartReportDateUnixMilli, *params.StartTimeSelection, params.StartTimeSelectionReference)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}

	startResults, err := dao.ListAwsAthenaWorkGroups(c, *startSelectedTime, params.AccountId, params.Region, nil, nil)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}

	endSelectedTime, err := dao.GetReferencedAwsAthenaWorkGroupReportTime(c, params.EndReportDateUnixMilli, *params.EndTimeSelection, params.EndTimeSelectionReference)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}

	endResults, err := dao.ListAwsAthenaWorkGroups(c, *endSelectedTime, params.AccountId, params.Region, nil, nil)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
...
// DiffSingleWorkGroup godoc
// @Summary      Diff WorkGroup
// @Description  get a diff of WorkGroup between two points in time
// @Tags         aws athena
// @Produce      json
// @Param        start_report_date query string true  "Which date to pull data from. Current date by default" Format(date)
// @Param		 start_time_selection query string false  "How to select the time range to pull data from. 'latest' by default" Enums(latest, before, after, at)
...
// @Success      200  {array}   routes.Diff
// @Failure      400
// @Router       /diff/aws/athena/work_groups/{name} [get]
func DiffSingleWorkGroup(c *gin.Context, dao db.ReaderDAO) {
	...
	startObject, err := dao.GetAwsAthenaWorkGroup(c, *startSelectedTime, id)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
	...
	endObject, err := dao.GetAwsAthenaWorkGroup(c, *endSelectedTime, id)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
	...
	if startObject == nil && endObject == nil {
		c.AbortWithStatusJSON(404, gin.H{"error": "No WorkGroup found with name " + id})
		return
	} else {
