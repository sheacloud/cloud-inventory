...
// @Failure      400
// @Router       /metadata/aws/backup/plans [get]
func GetBackupPlansMetadata(c *gin.Context, dao db.DAO) {
	...
	reportTimes, err := dao.AWS().Backup().GetBackupPlanReportTimes(c, reportDate)
	if err != nil {
...
// @Failure      400
// @Router       /inventory/aws/backup/plans [get]
func ListBackupPlans(c *gin.Context, dao db.DAO) {
	...
	selectedTime, err := dao.AWS().Backup().GetReferencedBackupPlanReportTime(c, params.ReportDateTime, *params.TimeSelection, params.TimeSelectionReference)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}

	results, err := dao.AWS().Backup().ListBackupPlans(c, *selectedTime, params.AccountId, params.Region, nil, nil)
	if err != nil {
...
// @Failure      400
// @Failure 	 404
// @Router       /inventory/aws/backup/plans/{backup_plan_arn} [get]
func GetBackupPlan(c *gin.Context, dao db.DAO) {
	var params routes.AwsQueryParameters
	if err := c.BindQuery(&params); err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	...
	result, err := dao.AWS().Backup().GetBackupPlan(c, *selectedTime, id)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}
...
// @Failure      400
// @Router       /diff/aws/backup/plans [get]
func DiffMultiBackupPlans(c *gin.Context, dao db.DAO) {
	...
	startSelectedTime, err := dao.AWS().Backup().GetReferencedBackupPlanReportTime(c, params.StartReportDateTime, *params.StartTimeSelection, params.StartTimeSelectionReference)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}

	startResults, err := dao.AWS().Backup().ListBackupPlans(c, *startSelectedTime, params.AccountId, params.Region, nil, nil)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}

	endSelectedTime, err := dao.AWS().Backup().GetReferencedBackupPlanReportTime(c, params.EndReportDateTime, *params.EndTimeSelection, params.EndTimeSelectionReference)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}

	endResults, err := dao.AWS().Backup().ListBackupPlans(c, *endSelectedTime, params.AccountId, params.Region, nil, nil)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
...
// @Success      200  {array}   routes.Diff
// @Failure      400
// @Router       /diff/aws/backup/plans/{backup_plan_arn} [get]
func DiffSingleBackupPlan(c *gin.Context, dao db.DAO) {
	var params routes.AwsDiffParameters
	if err := c.BindQuery(&params); err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	...
	startObject, err := dao.AWS().Backup().GetBackupPlan(c, *startSelectedTime, id)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
	...
	endObject, err := dao.AWS().Backup().GetBackupPlan(c, *endSelectedTime, id)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
