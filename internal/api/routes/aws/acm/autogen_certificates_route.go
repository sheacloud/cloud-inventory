...
type ListCertificatesResponse struct {
	Certificates    []*acm.Certificate `json:"certificates"`
	PaginationToken *string            `json:"pagination_token,omitempty"`
}

// GetCertificatesMetadata godoc
// @Summary      Get Certificates Metadata
// @Description  get a list of certificates metadata
// @Tags         aws acm
// @Produce      json
...
// @Failure      400
// @Router       /metadata/aws/acm/certificates [get]
func GetCertificatesMetadata(c *gin.Context, dao db.ReaderDAO) {
	...
	reportTimes, err := dao.GetAwsACMCertificateReportTimes(c, reportDate.UnixMilli())
	if err != nil {
...
// @Failure      400
// @Router       /inventory/aws/acm/certificates [get]
func ListCertificates(c *gin.Context, dao db.ReaderDAO) {
	...
	selectedTime, err := dao.GetReferencedAwsACMCertificateReportTime(c, params.ReportDateUnixMilli, *params.TimeSelection, params.TimeSelectionReference)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}

	results, err := dao.ListAwsACMCertificates(c, *selectedTime, params.AccountId, params.Region, nil, nil)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}

	c.IndentedJSON(200, ListCertificatesResponse{
...
// @Description  Get a specific Certificate by its CertificateArn
// @Tags         aws acm
// @Produce      json
// @Param        report_date query string false  "Which date to pull data from. Current date by default" Format(date)
// @Param        certificate_arn path string true "The certificate_arn of the Certificate to retrieve"
// @Param		 account_id query string false  "A specific account to pull data from. All accounts by default"
// @Param		 region query string false  "A specific region to pull data from. All regions by default"
// @Param		 time_selection query string false  "How to select the time range to pull data from. 'latest' by default" Enums(latest, before, after, at)
// @Param		 time_selection_reference query string false  "The reference time to use when selecting the time range to pull data from. Only used when time_selection is 'before', 'after', or 'at'." Format(dateTime)
// @Security     ApiKeyAuth
// @Success      200  {object}   acm.Certificate
// @Failure      400
// @Failure 	 404
// @Router       /inventory/aws/acm/certificates/{certificate_arn} [get]
func GetCertificate(c *gin.Context, dao db.ReaderDAO) {
	...
	id, err := url.QueryUnescape(c.Param("certificate_arn"))
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}
	...
	result, err := dao.GetAwsACMCertificate(c, *selectedTime, id)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}
...
// DiffMultiCertificates godoc
// @Summary      Diff Certificates
// @Description  get a diff of Certificates between two points in time
// @Tags         aws acm
// @Produce      json
...
// @Failure      400
// @Router       /diff/aws/acm/certificates [get]
func DiffMultiCertificates(c *gin.Context, dao db.ReaderDAO) {
	...
	startSelectedTime, err := dao.GetReferencedAwsACMCertificateReportTime(c, params.StartReportDateUnixMilli, *params.StartTimeSelection, params.StartTimeSelectionReference)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}

	startResults, err := dao.ListAwsACMCertificates(c, *startSelectedTime, params.AccountId, params.Region, nil, nil)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}

	endSelectedTime, err := dao.GetReferencedAwsACMCertificateReportTime(c, params.EndReportDateUnixMilli, *params.EndTimeSelection, params.EndTimeSelectionReference)
	if err != nil {
		c.AbortWithStatusJSON(400, gin.H{"error": err.Error()})
		return
	}

	endResults, err := dao.ListAwsACMCertificates(c, *endSelectedTime, params.AccountId, params.Region, nil, nil)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
...
// DiffSingleCertificate godoc
// @Summary      Diff Certificate
// @Description  get a diff of Certificate between two points in time
// @Tags         aws acm
// @Produce      json
// @Param        start_report_date query string true  "Which date to pull data from. Current date by default" Format(date)
// @Param		 start_time_selection query string false  "How to select the time range to pull data from. 'latest' by default" Enums(latest, before, after, at)
...
// @Success      200  {array}   routes.Diff
// @Failure      400
// @Router       /diff/aws/acm/certificates/{certificate_arn} [get]
func DiffSingleCertificate(c *gin.Context, dao db.ReaderDAO) {
	...
	startObject, err := dao.GetAwsACMCertificate(c, *startSelectedTime, id)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
	...
	endObject, err := dao.GetAwsACMCertificate(c, *endSelectedTime, id)
	if err != nil {
		c.AbortWithStatusJSON(500, gin.H{"error": err.Error()})
		return
	}
	...
	if startObject == nil && endObject == nil {
		c.AbortWithStatusJSON(404, gin.H{"error": "No Certificate found with certificate_arn " + id})
		return
	} else {
