...
func GenerateParquetGlueTerraform() {
	for _, service := range inventory.AwsCatalog {
		...
		for _, resource := range service.Resources {
			table, err := ConvertStructToParquetGlueTable(resource.ResourceModel, "aws", service.ServiceName, resource.ResourceName)
			if err != nil {
				panic(err)
			}
		...
		filename := fmt.Sprintf("./terraform/glue/autogen_aws_%s_parquet_glue_tables.tf", service.ServiceName)
