...
resource "aws_glue_catalog_table" "aws_s3_buckets" {
  ...
  storage_descriptor {
    ...
    columns {
      ...
      type       = "array<struct<status:string,abort_incomplete_multipart_upload:struct<days_after_initiation:int>,expiration:struct<days:int,expired_object_delete_marker:boolean,date:timestamp>,id:string,noncurrent_version_expiration:struct<noncurrent_days:int>,noncurrent_version_transitions:array<struct<noncurrent_days:int,storage_class:string>>,prefix:string,transitions:array<struct<days:int,storage_class:string,date:timestamp>>>>"
      comment    = ""
      parameters = {}
    }
    columns {
