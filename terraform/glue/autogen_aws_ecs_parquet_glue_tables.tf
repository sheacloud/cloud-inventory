...
resource "aws_glue_catalog_table" "aws_ecs_services" {
  ...
  storage_descriptor {
    ...
    columns {
      name       = "deployments"
      type       = "array<struct<capacity_provider_strategy:array<struct<capacity_provider:string,base:int,weight:int>>,created_at:timestamp,desired_count:int,failed_tasks:int,id:string,launch_type:string,network_configuration:struct<awsvpc_configuration:struct<subnets:array<string>,assign_public_ip:string,security_groups:array<string>>>,pending_count:int,platform_family:string,platform_version:string,rollout_state:string,rollout_state_reason:string,running_count:int,status:string,task_definition:string,updated_at:timestamp>>"
      comment    = ""
      parameters = {}
    }
    columns {
